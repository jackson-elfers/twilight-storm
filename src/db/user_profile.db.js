...
`;
  ...
  async search(data) {
    ...
    data.search_query.map((d, i) => {
      where += ` ${this.method.sqlstring.escape(`first_name like %${d}% or last_name like %${d}% or city like %${d}% or state like %${d}% or street like %${d}% or zip like %${d}% or title like %${d}% or keywords like %${d}% or `)} `;
    });
